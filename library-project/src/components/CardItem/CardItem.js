...
import {getUser, deleteBook, getAuthor ,getBook, deleteRecord} from "../../containers/utils/Api"
...
const CardItem = ({
 ...
 useEffect(() => {
  ...
  const catalogActions=userType==='admin'?[
  ...
  return (
    <ItemWrapper>
      ...
      > 
       ...
       {`Author: ${bookAuthor}`} <br />{`Book: ${book.name}`} <br />
     </p><Footer>
         <p style={{ fontWeight: "bold", fontSize: "16px" }}>
           Rent price:{`${reportBook.rentPrice}`} <br />
           Collateral price: {reportBook.collateralPrice} <br />

          {bookIsExpired()?<p style={{color:"red",fontWeight: "bold", fontSize: "16px" }}>Expired</p>:<p style={{color:"Green",fontWeight: "bold", fontSize: "16px" }}>Not expired</p>}
         </p> 
